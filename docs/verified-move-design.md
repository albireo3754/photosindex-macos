...
inventory. Assets judged personal, ambiguous, privacy-sensitive, or unsupported
remain in Photos.
...
- Copy failure: no deletion; retry stages or reuses verified files.
...
authoritative. For each date: sync, list fine groups, inspect every evidence
page, write classification records, move only confirmed assets, verify receipts,
then re-sync to prove only moved public IDs disappeared. An existing compatible
export may be reused only after its hashes and iCloud upload state are
revalidated.
